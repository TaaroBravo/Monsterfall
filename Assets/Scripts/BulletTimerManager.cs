...
public class BulletTimerManager : MonoBehaviour
{
    ...
    void Update()
    {
        var stunnedPlayers = allPlayers.Where(x => x != null).Where(x => x.stunnedByHit).Where(x => x.impactSpeed > 25);
        if (allPlayers.Count() <= 2)
        {
            var anyStunned = allPlayers.Where(x => x != null).Where(x => !x.stunnedByHit).Any(x => stunnedPlayers.Any(y => Vector3.Distance(y.transform.position, x.transform.position) < distance));
            slowTime = anyStunned;
        ...
        {
            var anyStunned = allPlayers.Where(x => x != null).Where(x => !x.stunnedByHit).Where(x => stunnedPlayers.Any(y => y.whoHitedMe != x)).Any(x => stunnedPlayers.Any(y => Vector3.Distance(y.transform.position, x.transform.position) < distance));
            slowTime = anyStunned;
